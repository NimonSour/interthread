...
// use crate::model::{self,name,GroupModelSdpl,get_ident_type_generics,MpscChannel,Vars,Cont,method,generic,live_static_method,Lib,Model,ActorAttributeArguments,GroupAttributeArguments};
// use crate::error;
...
//               mut met_new) =
//              method::get_methods( &actor_type,item_impl.clone(),aaa.assoc ,Model::Group.eq(&mac));
...
//         let vars = &Vars::new(&aaa,&actor_name,Model::Group,&mac) ;
...
//     {
//         // ADD FIELDS MEMBERS
//         let Cont{ direct_arms, script_fields,debug_arms,..} = &mut cont;
...
//             let member_script_name = name::script_group(&members[field].name);
...
//                 #script_name :: #arm_match => { msg.direct( &mut group. #field );} //#await_call  #error_send ;}
//             };
...
//     met_new extract Actor
//     met_new create  Actor
//     play    create  Actor
...
//     // add self_sdpl in collection of members
//     members.insert(cust_name.clone(),self_sdpl);
...
//     gaa: GroupAttributeArguments, 
//     item_impl: ItemImpl )  -> ( TokenStream, TokenStream ) {

//         // todo!("crate::model::group::macro_group_generate_code")

//     let mut grp_model = group_model( &gaa,&item_impl,Model::Actor,None);
    
//     let (mut code,edit) = grp_model.split_edit();
